...
class Imagev
{
    ...
    function Watermark($source, $target = '', $type = 'forum') {
        if (empty($this->param['watermarkstatus'])) {
            return $this->returncode(0);
        }
        ...
        $this->param['watermarkfile'][$type] = './static/image/common/'.($this->param['watermarktype'] == 'png' ? 'watermark.png' : 'watermark.gif');
        if(!is_readable($this->param['watermarkfile'][$type]) || ($this->param['watermarktype'] == 'text' && (!file_exists($this->param['watermarktext']['fontpath'][$type]) || !is_file($this->param['watermarktext']['fontpath'][$type])))) {
    ...
    function init($method, $source, $target, $nosuffix = 0) {
        ...
        if(isset($parse['host'])) {
            ...
            $this->tmpfile = $source = tempnam(static::$fileroot.'temp/', 'tmpimg_');
        ...
        if($method == 'thumb') {
            $target = empty($target) ? (!$nosuffix ? $this->getimgthumbname($source) : $source) : static::$fileroot.'./'.$target;
        } elseif($method == 'watermask') {
            $target = empty($target) ?  $source : static::$fileroot.'./'.$target;
        }
        $targetpath = dirname($target);
        cacheFileHelpers::mkdirv($targetpath);
    ...
    function Watermark_GD($type = 'forum') {
        ...
        if($this->param['watermarktype'] != 'text') {
            ...
            $watermark_logo	= $this->param['watermarktype'] == 'png' ? @imageCreateFromPNG($this->param['watermarkfile'][$type]) : @imageCreateFromGIF($this->param['watermarkfile'][$type]);
            if(!$watermark_logo) {
                return 0;
            }
    ...
    function Watermark_IM($type = 'forum') {
        ...
        if($this->param['watermarktype'] != 'text') {
            ...
            if($this->param['watermarktype'] != 'png' && $this->param['watermarktrans'][$type] != '100') {
