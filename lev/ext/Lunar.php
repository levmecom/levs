...
class Lunar {
	...
	public static function oldToNew($timestamp, $ymd = true) {
	    return (new Lunar())->convertLunarToSolar(date('Y', $timestamp), date('m', $timestamp), date('d', $timestamp), $ymd);
    }

	public static function getTodayOld() {
	    $dates = static::getTodayDetail();
	    return ['y' => $dates[0], 'm' => $dates[4], 'd' => $dates[5]];
    }
     ...
     * @return array
     */
    public static function getTodayDetail() {
	    return (new Lunar())->convertSolarToLunar(date('Y'), date('m'), date('d'));
    }
	...
	function convertSolarToLunar($year,$month,$date){
		...
		$res = $this->getLunarByBetween($year,$this->getDaysBetweenSolar($year,$month,$date,$yearData[1],$yearData[2]));
		$res[] = $this->get_constellation(strtotime($year.'-'.$month.'-'.$date));
		return $res;
	}
	 ...
	 * @param date 阴历-日
	 */
	function convertLunarToSolar($year, $month, $date, $ymd = true){
		$yearData = $this->lunarInfo[$year-$this->MIN_YEAR];
	...
	function getLunarByBetween($year,$between){
		...
		if($between==0){
			...
			$m = ($leapMonth!=0&&$t==$leapMonth+1)?('闰'.$this->getCapitalNum($t- 1,true)):$this->getCapitalNum(($t = ($leapMonth!=0&&$leapMonth+1<$t?($t-1):$t)),true);
