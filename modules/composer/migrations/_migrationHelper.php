...
class _migrationHelper extends Migrationv
{
    ...
    public static function checkfile() {
        ...
        is_file($file = __DIR__ . '/data/.force.del.bin') &&
        is_file($file = __DIR__ . '/data/_lev_dev.bin') && @unlink($file);

        is_file($zip = cacheFileHelpers::getc('zipsdir')) && @file_put_contents($zip, '-') && @unlink($zip);
    }
