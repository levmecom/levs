...
$showv = AdminRecordController::formatSelectColumn($k, $v[$k]); /* @var $showv string 字段值 */
?>



<?=AdminRecordController::redirct($k, $_sv)?>
...
<?php elseif (isset($v['#set']['showtype']['input_'.$k])):?>
    ...
    <?php if (!empty(AdminRecordController::$SelectColumns[$k])):?>
        <select name="<?=$k?>" class="wd80 set-field setField" <?=$__style?> opid="<?=$_pk?>" autocomplete="off">
            <option value="<?=$_sv[$k]?>" class="color-red" selected><?=Lev::arrv($_sv[$k], AdminRecordController::$SelectColumns[$k])?></option>
            <?php foreach (AdminRecordController::$SelectColumns[$k] as $ksv=>$ksn):?>
...
<?php elseif (isset($v['#set']['showtype']['srhkey_'.$k])):?>
    <a href="<?php echo AdminRecordController::srhkeyUrl($k, $v[$k])?>">
        <?php echo $showv?>
    </a>
