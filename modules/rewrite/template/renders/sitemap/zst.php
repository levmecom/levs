    ...
    <div class="page-content">
        <div class="page-content-inner">
            ...
            <form id="saveForm" action="" method="post">
                ...
                <div class="card-header card">
                    ...
                    <div class="item-input">
                        <select onchange="this.value && (window.location = this.value)">
                            <option value="<?=Lev::toCurrent(['code'=>null])?>">切换采种</option>
                            <?php foreach ($lotts as $v):?>
                            <option value="<?=Lev::toCurrent(['code'=>$v['code']])?>" <?=$code == $v['code']?'selected':''?>><?=$v['lottname']?></option>
                            <?php endforeach;?>
                        </select>
                    </div>
                    <div class="item-input">
            ...
            <div class="list-block card">
                <ul>
                    <li class="list-group item-content">彩票工具地址</li>
                    <?php foreach ($lottTools as $v):?>
                        <?php $_href = $v['url']?>
                        <li>
                    ...
                    <?php endforeach;?>
                    <li class="list-group item-content">彩票开奖地址</li>
                    <?php foreach ($lotts as $v):?>
                        <?php $_href = \modules\levssq\helpers\UrlssqHelper::view($v['code'])?>
                        <li>
                            <label class="item-content item-link item-content-32">
                                <div class="item-media">
                                    <input type="checkbox" name="urls[]" value="<?=$_href?>">
                    ...
                    <?php endforeach;?>
                    <?php foreach ($urls as $_type => $r):?>
                    <li class="list-group item-content"><?=$r['title']?></li>
                    <?php foreach ($r['tmps'] as $_tmp => $v):?>
                        <?php $_href = \modules\levssq\modules\zst\helpers\UrlZstHelper::view($code, $_type, $_tmp)?>
                    <li>
                        <label class="item-content item-link item-content-32">
                            <div class="item-media">
                                <input type="checkbox" name="urls[]" value="<?=$_href?>">
