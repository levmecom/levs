...
class _migrationHelper extends Migrationv
{
    ...
    public static function getModuleTableColumnsComment() {
        $tables = _install::getTables();
        $datas = [];
        if ($tables) {
    ...
    public static function deleteInstallFile() {
        if (!static::isLevDev()) {
            @unlink(__DIR__ . '/_update.php') &&
            is_file($file = __DIR__ . '/_install.php') && @unlink($file);
            is_file($file = __DIR__ . '/_migrationHelper.php') && @unlink($file);
            ...
            is_file($file = dirname(__DIR__) . '/web/install_lev.php') && @unlink($file);
            is_file($file = dirname(__DIR__) . '/web/gate.php') &&
            file_put_contents(dirname(__DIR__) . '/web/levs.php', file_get_contents($file), LOCK_EX) && @unlink($file);
            if (empty(Lev::$app['isDiscuz'])) {
                is_file($file = dirname(__DIR__) . '/discuz_plugin_levs.xml') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/discuz_plugin_levs_SC_GBK.xml') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/discuz_plugin_levs_SC_UTF8.xml') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/hookv.class.php') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/hookv_mobile.class.php') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/iden.const.php') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/levs.inc.php') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/superman.inc.php') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/uninstall.php') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/upgrade.php') && @unlink($file);
                is_file($file = dirname(__DIR__) . '/web/superman.php') && @unlink($file);
            }
        }
    }
